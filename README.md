...
**Google Photos captions**: There is no way to programmatically set a photo description that Google Photos will display in its UI. We write captions as EXIF `ImageDescription` and XMP `dc:description`, but Google Photos shows them under "Other" instead of the description field. The Google Photos Library API accepts a `description` field, but it is only visible via the API — not in the web or mobile UI. Apple Photos displays XMP descriptions correctly.
